            ...
            <div class="main-grid">
                                            ...
                                            <c:forEach var="acc" items="${accounts}">
                                                <tr>
                                                    <td class="table-title-cell" style="font-family: monospace; font-size: 1rem; letter-spacing: 0.05em;">
                                                        <a href="${pageContext.request.contextPath}/account/details?id=${acc.accountId}" style="color:var(--primary-color); text-decoration:none; font-weight:600;">
                                                            ${acc.accountNumber}
                                                        </a>
                                                    </td>
                                                    <td>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td style="font-weight: 700; font-size: 0.95rem;
                                                            <c:choose>
                                                                ...
                                                                <c:when test="${txn.transactionType == 'TRANSFER' && txn.receiverAccount == contextAccountNumber}">color: var(--success-color);</c:when>
                                                                <c:otherwise>color: var(--danger-color);</c:otherwise>
                                                            </c:choose>">
                                                            <c:choose>
                                                                <c:when test="${txn.transactionType == 'DEPOSIT'}">+</c:when>
                                                                <c:when test="${txn.transactionType == 'TRANSFER' && txn.receiverAccount == contextAccountNumber}">+</c:when>
                                                        ...
                                                        <td class="table-sub-cell">
                                                            <c:choose>
                                                                <c:when test="${txn.transactionType == 'TRANSFER' && txn.senderAccount == contextAccountNumber}">
                                                                    Transferred To: ${txn.receiverAccount}
                                                                </c:when>
                                                                <c:when test="${txn.transactionType == 'TRANSFER' && txn.receiverAccount == contextAccountNumber}">
                                                                    Received From: ${txn.senderAccount}
                                                                </c:when>
                                                                <c:otherwise>
                                                                    Self Transaction
                                                                </c:otherwise>
                ...
                <div class="column-side" style="gap: 2rem; display: flex; flex-direction: column;">
                    ...
                    <c:choose>
                        <c:when test="${empty accounts}">
                            <div class="card">
                                <h3 class="op-card-title">Getting Started</h3>
                                <div style="margin-bottom:0.75rem;">
                                    <p style="font-size:0.85rem; color:var(--text-secondary);">Follow these steps to start banking with FinTrust.</p>
                                </div>
                                <div class="getting-started-step">
                                    <div class="step-number">1</div>
                                    <div>
                                        <div style="font-weight:600; font-size:0.9rem; color:var(--text-primary);">Open an Account</div>
                                        <div style="font-size:0.75rem; color:var(--text-secondary);">Choose Savings or Current account above</div>
                                    </div>
                                </div>
                                <div class="getting-started-step">
                                    <div class="step-number">2</div>
                                    <div>
                                        <div style="font-weight:600; font-size:0.9rem; color:var(--text-primary);">Wait for Approval</div>
                                        <div style="font-size:0.75rem; color:var(--text-secondary);">Your account will be verified by our team</div>
                                    </div>
                                </div>
                                <div class="getting-started-step">
                                    <div class="step-number">3</div>
                                    <div>
                                        <div style="font-weight:600; font-size:0.9rem; color:var(--text-primary);">Start Banking</div>
                                        <div style="font-size:0.75rem; color:var(--text-secondary);">Deposit, transfer, withdraw and more</div>
                                    </div>
                                </div>
                            </div>
                        </c:when>
                                ...
                                <div style="display:flex; flex-direction:column; gap:1rem;">
                                    <div style="display:flex; align-items:center; gap:0.75rem;">
                                        ...
                                        <div>
                                            ...
                                            <div style="font-weight:700; font-size:0.95rem; color:var(--text-primary);">${primaryAccount.accountType}</div>
                                        </div>
                                    </div>
                                    <div style="display:flex; align-items:center; gap:0.75rem;">
                                        ...
                                        <div>
                                            ...
                                            <div style="font-weight:700; font-size:1.1rem; color:var(--success-color);">₹<fmt:formatNumber value="${primaryAccount.balance}" pattern="#,##0.00"/></div>
                                        </div>
                                    </div>
                                    <div style="display:flex; align-items:center; gap:0.75rem;">
                                        ...
                                        <div>
                                            ...
                                            <span class="badge
                                                <c:choose>
                                                    <c:when test="${primaryAccount.status == 'ACTIVE'}">badge-success</c:when>
                                                    <c:when test="${primaryAccount.status == 'PENDING'}">badge-warning</c:when>
                                                    <c:otherwise>badge-danger</c:otherwise>
                                                </c:choose>">
                                    ...
                                    <div style="display:flex; align-items:center; gap:0.75rem;">
                                        ...
                                        <div>
                                            <div style="font-size:0.7rem; font-weight:600; color:var(--text-secondary); text-transform:uppercase; letter-spacing:0.05em;">Total Transactions</div>
                                            <div style="font-weight:700; font-size:0.95rem; color:var(--text-primary);">${totalTransactionCount}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
